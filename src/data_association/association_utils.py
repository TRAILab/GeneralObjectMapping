...
from src.data_association.coco_define import COCO_METAINFO
from src.data_association.detection import Detection
from src.data_association.init_pose_estimator import *
from src.data_association.object_refiner import *
from src.data_association.scan_matcher import registration, sm_evaluator
...
class Object:
    def __init__(self, obj_id, obs):
        ...
        if (
            ...
            and self.bbox_length is not None
        ):
    ...
    def merge(self, obj):
        ...
        for past_obs_name_class in obj.past_obs_name_classes.keys():
            if past_obs_name_class in self.past_obs_name_classes.keys():
                self.past_obs_name_classes[past_obs_name_class] += obj.past_obs_name_classes[
                    past_obs_name_class
                ]
                self.past_obs_name_classes_count[
                    past_obs_name_class
                ] += obj.past_obs_name_classes_count[past_obs_name_class]
            else:
                self.past_obs_name_classes[past_obs_name_class] = obj.past_obs_name_classes[
                    past_obs_name_class
                ]
                self.past_obs_name_classes_count[past_obs_name_class] = (
                    obj.past_obs_name_classes_count[past_obs_name_class]
                )
    ...
    def update_class_and_score(self):
        ...
        self.score = (
            self.past_obs_name_classes[self.get_most_likely_class()]
            / self.past_obs_name_classes_count[self.get_most_likely_class()]
        )
...
class Scene:
    ...
    def associate_objects(self):
        ...
        if self.n_objects == 0:
            for i in range(self.frames[-1].n_obs):
                print(
                    "Observation {}:{} is not associated with any object".format(
                        i, self.frames[-1].observations[i].name_class
                    )
                )
        ...
        for idx in range(len(col_ind)):
            ...
            if score != self.assoc_cost_default:
                print(
                    "    Observation {}:{} is associated with object {}:{} with score {}".format(
                        obs_idx, obs_class, obj_idx, obj_class, score
                    )
                )
    ...
    def calulate_affinity(self):
        ...
        cost_matrix_fitness = self.assoc_cost_default * np.ones(
            (self.frames[-1].n_obs, self.n_objects)
        )
        ...
        for i, obs in enumerate(self.frames[-1].observations):
            for j, obj in enumerate(self.objects):
                ...
                fitness, rmse, trans = registration(
                    obs.point_cloud, obj.point_cloud, 0.07, draw=False
                )
        ...
        weight_matrix = (
            weight_fitness * cost_matrix_fitness
            + weight_rmse * cost_matrix_rmse
            + weight_trans * cost_matrix_trans
        )
                    ...
                    if overlap_volume > 0.5:
                        ...
                        if (0 < rmse < 0.08) and (fitness > 0.6):
                            if obj1.volume < obj2.volume:
                                ...
                                print(
                                    "    Merging object {}:{} into object {}:{} with fitness {}".format(
                                        i, obj1.name_class, j, obj2.name_class, fitness
                                    )
                                )
                            else:
                                obj1.merge(obj2)
                                ...
                                print(
                                    "    Merging object {}:{} into object {}:{} with fitness {}".format(
                                        j, obj2.name_class, i, obj1.name_class, fitness
                                    )
                                )
    ...
    def visualize_frames(self):
        ...
        for i in range(self.n_frames):
            for j in range(self.frames[i].n_obs):
                ...
                vis.add_geometry(
                    "PCD_F_" + str(i) + "_O_" + str(j), self.frames[i].observations[j].point_cloud
                )
                vis.add_geometry(
                    "BOX_F_" + str(i) + "_O_" + str(j), self.frames[i].observations[j].bbox_3d
                )
                vis.add_3d_label(
                    self.frames[i].observations[j].bbox_center
                    + [0, self.frames[i].observations[j].bbox_height, 0],
                    self.frames[i].observations[j].name_class,
                )
    ...
    def visualize_objects(self, vis=None):
        ...
        for i in range(self.n_objects):
            ...
            add_o3d_geometry(
                vis, "PCD_OBJ_" + str(obj_id), self.objects[i].point_cloud, b_valid_gui
            )
            ...
            if self.objects[i].has_est_pose:
                add_o3d_geometry(
                    vis,
                    "GT_OBJ_" + str(obj_id),
                    self.pose_estimator.model_library.get_transformed_model(self.objects[i]),
                    b_valid_gui,
                )

            if b_valid_gui:
                vis.add_3d_label(
                    self.objects[i].bbox_center + [0, self.objects[i].bbox_height, 0],
                    self.objects[i].name_class + " ID: " + str(obj_id),
                )
