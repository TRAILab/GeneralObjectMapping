...
class Detection:
    def __init__(self, object, obs_idx):
        ...
        self.det.T_cam_deepsdf = None
        self.det.T_obj_deepsdf = None
        ...
        self.det.surface_points_world = (
            np.hstack((self.det.surface_points, np.ones((self.det.num_surface_points, 1))))
            @ self.det.T_world_cam.T
        )[:, 0:3]
        ...
        """
        self.det.T_obj_deepsdf = self.get_t_obj_deepsdf(object)
        self.det.T_cam_deepsdf = (self.det.T_cam_object @ self.det.T_obj_deepsdf).astype(np.float32)
    ...
    def _generate_rays(self, depth, K, mask):
        """
        Previously, the lidar points are depth observations
