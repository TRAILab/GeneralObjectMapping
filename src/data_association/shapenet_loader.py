...
class Dataset(data.Dataset):
    ...
    def get_path(self, type):
        ...
        if self.load_name:
            paths_json = [
                os.path.join(self.root, type + str(i) + "_id2name.json") for i in range(len(paths))
            ]
            self.path_name_all += paths_json
        if self.load_file:
            paths_json = [
                os.path.join(self.root, type + str(i) + "_id2file.json") for i in range(len(paths))
            ]
...
if __name__ == "__main__":
    ...
    d = Dataset(
        root=root, dataset_name=dataset_name, class_choice=class_name, num_points=2048, split=split
    )
    print("datasize:", d.__len__())

    # for i in range(0,  d.__len__(),1):
    for i in range(4, 5):
        ps, lb, n, f = d[i]
        print(n)
        if n == class_name:
            print("index:", i)
            chair_array = ps.numpy()

            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(chair_array)
            o3d.visualization.draw_geometries([pcd])
            pcd_dim = pcd.get_max_bound() - pcd.get_min_bound()
            print("pcd_dim:", pcd_dim)
            # break
            # save the pcd as a .ply file
            o3d.io.write_point_cloud(class_name + ".ply", pcd)
