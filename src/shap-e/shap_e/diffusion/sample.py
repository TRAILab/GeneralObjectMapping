...
def sample_latents(
    *,
    ...
    if hasattr(model, "cached_model_kwargs"):
        ...
        if "clip_embedding_offset" in model_kwargs:
            clip_embedding_offset = model_kwargs["clip_embedding_offset"]

            # remove it
            del model_kwargs["clip_embedding_offset"]

        """
        This function gets embeddings from images;
        We further add a check, if embeddings is given and images is empty, we skip this process.
        """
        if "embeddings" in model_kwargs and "images" not in model_kwargs:
            pass
        ...
        # if not None
        if clip_embedding_offset is not None:
            # update this offset to CLIP feature
            # add a batch dimension
            clip_embedding_offset = clip_embedding_offset.unsqueeze(0).repeat(batch_size, 1, 1)

            model_kwargs["embeddings"] = (
                model_kwargs["embeddings"] + clip_embedding_offset
            )  # shape: batch, 1024, 256
