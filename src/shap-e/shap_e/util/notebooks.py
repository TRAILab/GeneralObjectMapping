...
def decode_latent_images(
    ...
    decoded = xm.renderer.render_views(
        ...
        options=AttrDict(
            rendering_mode=rendering_mode, render_with_direction=False, background=background
        ),
    )
...
def decode_latent_images_with_grad(
    xm: Union[Transmitter, VectorDecoder],
    latent: torch.Tensor,
    cameras: DifferentiableCameraBatch,
    rendering_mode: str = "stf",
    ...
    decoded = xm.renderer.render_views(
        AttrDict(cameras=cameras, ray_batch_size=ray_batch_size),
        params=(xm.encoder if isinstance(xm, Transmitter) else xm).bottleneck_to_params(
            latent[None]
        ),
        options=AttrDict(
