...
def render_views_from_rays(
    ...
    # TODO: Speed up rendering; only consider pixels inside projected bbox areas, instead of whole image area
    ...
    n_batches = (rays.shape[1] + ray_batch_size - 1) // ray_batch_size
    ...
    for idx in range(n_batches):
        ...
        end_idx = min((idx + 1) * ray_batch_size, rays.shape[1])
        rays_batch = AttrDict(
        ...
        if isinstance(camera, DifferentiableProjectiveCamera):
            ...
            z_batch = z_directions[:, start_idx:end_idx]
